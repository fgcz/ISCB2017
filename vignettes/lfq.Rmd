...
```{r fig.retina=3}
library(lattice)
xyplot(peptide ~ rt | filename, group=grepl('GE', filename),
       data = peptide.rt)
```


```{r}
pmax <- aggregate(score ~ filename + peptide, FUN=max, data=peptide.rt)
...
XIC.rt.filtered <- XIC.rt[XIC.rt$rt.y - eps < XIC.rt$rt.x & XIC.rt$rt.x < XIC.rt$rt.y + eps, ] 
...
library(gplots)
heatmap.2(asinh(as.matrix((M))), margins = c(20,15), trace = "none")
